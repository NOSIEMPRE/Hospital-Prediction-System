...
After training, the model is saved **twice**:
...
async def lifespan(app: FastAPI):
    ...
    # 1. Read the run_id for traceability
    run_id_path = Path("run_id.txt")
    if run_id_path.exists():
        RUN_ID = run_id_path.read_text().strip()

    # 2. Load the model from the saved MLflow directory
...
```
The `model` and `RUN_ID` are stored as module-level globals, accessible by all request handlers.
...
```
Used by Render to verify the container is alive.

**`POST /predict`** — Prediction:
- Input: JSON body matching `PatientRequest` schema (20 fields)
- Output: `{"risk_score": 0.342, "model_version": "abc123..."}`

### Request Validation with Pydantic

Pydantic automatically validates every incoming request. If a field is missing or out of range, FastAPI returns a `422 Unprocessable Entity` error before the prediction code is even reached:
```python
class PatientRequest(BaseModel):
    time_in_hospital: int = Field(..., ge=1, le=14)   # must be 1–14
    num_lab_procedures: int = Field(..., ge=0)          # must be ≥ 0
    age: str = Field(...)                               # must be a string
    # ... 17 more fields
```
...
def predict(patient: PatientRequest):
    ...
    feature_dict = patient.model_dump()                       # Pydantic → dict
    pred_proba = model.predict_proba([feature_dict])[0, 1]   # Pipeline inference

    return PredictionResponse(
        risk_score=float(pred_proba),           # 0.0 – 1.0
        model_version=RUN_ID or "unknown",
    )
```
The Pipeline's `predict_proba` runs the **entire pipeline**: vectorizer → XGBoost → probability. The score is the probability of the positive class (readmission within 30 days).
...
│  8. UNIT TESTS: pytest tests/test_train.py                   │
...
│ 12. INTEGRATION TESTS: pytest tests/test_api.py             │
...
The `streamlit_app.py` frontend is configured to call the Render URL by default:
```python
DEFAULT_API = "https://hospital-prediction-system.onrender.com"
```
Users can override this in the sidebar to point to `http://localhost:9696` for local development.

### Local vs Production Comparison

| Aspect | Local | Render (Production) |
|--------|-------|---------------------|
| URL | `http://localhost:9696` | `https://hospital-prediction-system.onrender.com` |
...
│  - Serves: GET /health · POST /predict                              │
│                                                                     │
│  Public URL: hospital-prediction-system.onrender.com               │
└────────────────────────────┬────────────────────────────────────────┘
                             │ HTTP POST /predict
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│                      USER INTERFACE                                 │
│  streamlit_app.py                                                   │
│  - Sends patient data to /predict                                   │
│  - Displays risk score (0–1) as gauge chart                        │
│  - Can point to localhost or Render URL                             │
└─────────────────────────────────────────────────────────────────────┘
```
...
*Stack: XGBoost · FastAPI · MLflow · Docker · GitHub Actions · Render*
